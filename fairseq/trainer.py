...
#                print('| NOTICE: your device may support faster training with --fp16')
    ...
    def word_train_step(self, samples, word_log_outputs, dummy_batch=False):
        """Do forward, backward and parameter update."""
        self._set_seed()
        self.model.train()
        self.criterion.train()
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                loss_idx = i
                # forward and backward
                loss, sample_size, logging_output = self.task.train_word_da_step(
                    word_log_outputs, loss_idx, sample, self.model, self.criterion, self.optimizer,
                    ignore_grad
                )
    ...
    def word_valid_step(self, sample, word_log_outputs, raise_oom=False):
        """Do forward pass in evaluation mode."""
        with torch.no_grad():
            self.model.eval()
            self.criterion.eval()
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_word_da_step(
                    sample, word_log_outputs, self.model, self.criterion, self.optimizer
                )
            except RuntimeError as e:
                if 'out of memory' in str(e) and not raise_oom:
                    print('| WARNING: ran out of memory, retrying batch')
