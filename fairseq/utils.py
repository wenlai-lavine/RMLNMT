...
def validate(args, trainer, task, epoch_itr, subsets, train_progress=None):
    ...
    for subset in subsets:
        ...
        # ipdb.set_trace()
        flag = subset.endswith('dev')
        if flag:
            tmp_subset = 'query_dev'
            task.load_dataset('query_dev')
            dev_datasets = task.dataset('query_dev')
        else:
            tmp_subset = subset
            task.load_dataset(subset)
            dev_datasets = task.dataset(subset)
        # ipdb.set_trace()
        itr = task.get_batch_iterator(
            dataset=dev_datasets,
            max_tokens=args.max_tokens,
        ...
        progress = progress_bar.build_progress_bar(
            ...
            prefix='valid on \'{}\' subset'.format(tmp_subset),
...
def bucket_sgd(task, args, model, criterion, bucket, train_function, cur_epoch, last_trainer=None):
    ...
    while lr > args.min_lr and inner_epoch < max_inner_epoch and trainer.get_num_updates() < max_update:
        ...
        # new_sate_dict = {k: v.cuda() for k, v in new_state.items() if k in pre_dict.keys()}
        # ### model name same but shape different, we 扩展tensor（与general保持一致）
        # expand_dict = dict()
        # for (new_key, new_val), (pre_key, pre_val) in zip(new_sate_dict.items(), pre_dict.items()):
        #     keep_val = pre_val.cuda()
        #     # print('keep_val type: ')
        #     # print(type(keep_val))
        #     split_tmp = new_val.cuda().chunk(7, dim=0)
        #     # print('split_tmp type: ')
        #     # print(type(split_tmp))
        #     expand_val = torch.cat([keep_val, split_tmp[6]])
        #     expand_dict[new_key] = expand_val
        # ### 更新字典
        # model.state_dict().update(expand_dict)
...
def word_sgd(task, args, model, criterion, train_function, word_da_loss):
    import math
    from fairseq.trainer import Trainer
    ...
    while lr > args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update:
        # Train the model for one epoch
        batch_info += train_function(args=args, trainer=trainer, task=task, epoch_itr=epoch_itr, is_curriculum=args.is_curriculum, word_log_outputs=word_da_loss)
        # Evaluate on validation split
        if epoch_itr.epoch % args.validate_interval == 0:
            valid_losses, valid_stats = validate(args, trainer, task, epoch_itr, valid_subsets)
        # save checkpoint
...
def sgd(task, args, model, criterion, train_function):
    import math
    from fairseq.trainer import Trainer
    flag = args.train_subset.endswith('dev')
    if flag:
        task.load_dataset('support_dev')
        train_dataset = task.dataset('support_dev')
    else:
        task.load_dataset(args.train_subset)
        train_dataset = task.dataset(args.train_subset)
    print("| train on {} sents".format(len(train_dataset)))
    ...
    dummy_batch = train_dataset.get_dummy_batch(num_tokens=args.max_tokens, max_positions=max_positions)
    if flag:
        oom_batch = task.dataset('support_dev').get_dummy_batch(1, max_positions)
    else:
        oom_batch = task.dataset(args.train_subset).get_dummy_batch(1, max_positions)
    # Create a trainer for training the model
    trainer = Trainer(args, task, model, criterion, dummy_batch, oom_batch)
...
# TODO this is also copied
def word_train(args, trainer, task, epoch_itr, is_curriculum, word_log_outputs):
    import collections
    import math
    from fairseq.data import iterators
    from fairseq import progress_bar
    ...
    batch_info = []
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        log_output = trainer.word_train_step(samples, word_log_outputs)
        if log_output is None:
            continue
