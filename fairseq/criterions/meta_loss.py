...
def build_single_word_task_loss(distributed_world_size, criterion):
    if distributed_world_size == 1:
        copy_step = deepcopy
        forward_step = noop
    else:
    ...
    if criterion == 'maml':
        ...
        assert criterion == 'fomaml_word'
        meta_loss_function = fomaml_word_loss
    return ComputeWordLoss(copy_step=copy_step, forward_step=forward_step, meta_loss_function=meta_loss_function)


class ComputeWordLoss(object):
    def __init__(self, copy_step, forward_step, meta_loss_function):
        self.copy_step = copy_step
        self.forward_step = forward_step
        self.meta_loss_function = meta_loss_function
        # self.word_da_loss = word_da_loss

    def __call__(self, train_task, model, word_da_loss):
        return self.meta_loss_function(train_task=train_task, model=model, copy_fn=self.copy_step,
                                       forward_fn=self.forward_step, word_da_loss=word_da_loss)
...
class MetaWordLoss(FairseqCriterion):

    def __init__(self, args, task):
        super().__init__(args, task)
        self.compute_loss = build_single_word_task_loss(distributed_world_size=args.distributed_world_size, criterion=args.criterion)

    def forward(self, model, sample, word_da_loss, reduce=True):
        ...
        for train_task in sample:
            ...
            single_loss, single_loss_stats = self.compute_loss(train_task=train_task, model=model, word_da_loss=word_da_loss)
...
def fomaml_word_loss(train_task, model, copy_fn, forward_fn, word_da_loss):
    args = train_task.args
    task_criterion = train_task.build_criterion(args)
    # SGD step
    fine_tuned_model = copy_fn(model)
    ...
    #                      train_function=utils.train)
    sgd_dict = utils.sgd(task=train_task, args=args, model=fine_tuned_model, criterion=task_criterion,
                         train_function=utils.word_train, word_da_loss=word_da_loss)
    loss_stats = get_loss_stats(sgd_dict=sgd_dict)
    forward_fn(model=model, train_task=train_task, task_criterion=task_criterion)
    # Meta-Loss computation
    subset = args.valid_subset.split(',')[0]
...
@register_criterion('fomaml_word')
class FoMamlWordLoss(MetaWordLoss):
