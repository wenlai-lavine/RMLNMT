...
@register_task('word_adapt_sampling')
class WordAdapt_sampling(TranslationTask):
            ...
            if not fine_tune:
                ...
                if 'dev' in split:
                    if int(self.args.domain_nums) == 6:
                        final_df.append(pd.concat([df_group_list[1]]))
                        final_df.append(pd.concat([df_group_list[0]]))
                        final_df.append(pd.concat([df_group_list[2]]))
                        final_df.append(pd.concat([df_group_list[3]]))
                        final_df.append(pd.concat([df_group_list[4]]))
                        final_df.append(pd.concat([df_group_list[5]]))
                    ...
                    if int(self.args.domain_nums) == 6:
                        final_df.append(pd.concat([df_group_list[1]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[0]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[2]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[3]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[4]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[5]] * 5).sample(frac=1))
                    elif int(self.args.domain_nums) == 5 and self.args.translation_task == 'en2de':
                        final_df.append(pd.concat([df_group_list[1]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[0]] * 5).sample(frac=1))
                        final_df.append(pd.concat([df_group_list[2]] * 5).sample(frac=1))
