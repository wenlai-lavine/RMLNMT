...
SEED = 7
SPM_MODEL_PATH = 'you sentencepiece model path'
...
def apply_roundrobin(domains, tasks_by_domain, cl_task_reorder=False, train_limit=100000, valid_limit=None, test_limit=None):
    ...
    if not cl_task_reorder:
        ...
        balanced_meta_train = sorted(balanced_meta_train, key=lambda x: x.task_score, reverse=True)
    # ipdb.set_trace()
...
def to_fairseq_word_task(task, task_score, args, src_dict, tgt_dict, sample_df):
    # task is a Tuple, first entry is the key
    task_args = deepcopy(args)
    # domain_task = task.task_group.any().split('.')[-1]
    task_args.train_subset = 'support_dev'
    task_args.valid_subset = 'query_dev'
    # task_args.test_subset = task_args.test_subset + '.' + domain_task
    fairseq_task = tasks.setup_task(args=task_args, user_data_frame=task, src_dict=src_dict, tgt_dict=tgt_dict, task_score=task_score, sample_df=sample_df)
    # fairseq_task.load_word_dataset('valid', combine=True)
    return fairseq_task

def to_fairseq_train_task(task, task_score, args, src_dict, tgt_dict):
    # task is a Tuple, first entry is the key
    task_args = deepcopy(args)
    domain_task = task.task_group.any().split('.')[-1]
    task_args.train_subset = task_args.train_subset + '.' + domain_task
    ...
    # task_args.test_subset = task_args.test_subset + '.' + domain_task
    fairseq_task = tasks.setup_task(args=task_args, user_data_frame=task, src_dict=src_dict, tgt_dict=tgt_dict,task_score=task_score)
    fairseq_task.load_dataset(task_args.train_subset)
    fairseq_task.load_dataset(task_args.valid_subset)
    # ipdb.set_trace()
    return fairseq_task


def get_info_table(data_dir, domain_group, split_subset, source_lang, target_lang, with_score=False):
    ...
    if with_score:
        ...
        info_df = pd.DataFrame({
            ...
            'tgt': target_sents,
            'score': sents_score,
            'domain_group': [domain_group for i in range(0, len(source_sents))],
...
def form_split_sents_df(data_dir, support_subset, query_subset, source_lang, target_lang, domain, dev_subset=None):
    if support_subset == None and query_subset == None and dev_subset != None:
        dev_sentences = get_info_table(data_dir=data_dir, domain_group=domain, split_subset=dev_subset, source_lang=source_lang, target_lang=target_lang, with_score=True)
        return MetaSplitData(DOMAIN=domain, support=None, query=None, dev=dev_sentences)
    else:
        support_sentences = get_info_table(data_dir=data_dir, domain_group=domain, split_subset=support_subset, source_lang=source_lang, target_lang=target_lang, with_score=True)
        query_sentences = get_info_table(data_dir=data_dir, domain_group=domain, split_subset=query_subset, source_lang=source_lang, target_lang=target_lang, with_score=True)
...
def load_static_meta_splits(root, domain, meta_train_subset, meta_dev_subset, meta_test_subset, downstream_train_subset, downstream_valid_subset, downstream_test_subset):
    meta_train_dir = os.path.join(root, domain, meta_train_subset)
    meta_dev_dir = os.path.join(root, domain, meta_dev_subset)
...
def split_domain_task(split_sents_df, support_tokens, query_tokens, domain, cl_sample=False, is_dev=False):
    ...
    if is_dev:
        ...
        if domain == 'WMT':
            df.task_group = ['support.' + domain + '_dev'] * 500 + ['query.' + domain + '_dev'] * (len(df) - 500)
        else:
    ...
    # ipdb.set_trace()
    if cl_sample:
        task_with_score.sort(key=lambda x : x[1], reverse=True)
    # TODO: PAUSE
    # ipdb.set_trace()
...
def assign_group_by_sentences_sampling(signature, args, info_df, cl_assign=False):
    if cl_assign:
        info_df = info_df.sort_values(by='score', ascending=False)

    split_size = int(info_df.shape[0] / args.task_nums)

    total_df_list = []

    for i in range(args.task_nums):
        if i != args.task_nums - 1:
            tmp_df = info_df[split_size * i: split_size * i + split_size]
            tmp_df.task_group = [signature + '_' + str(i)] * split_size
        else:
            tmp_df = info_df[split_size * i: info_df.shape[0]]
            tmp_df.task_group = [signature + '_' + str(i)] * (info_df.shape[0] - split_size * i)
        total_df_list.append(tmp_df)
    dl_df = pd.concat(total_df_list)
    return dl_df


def split_word_curriculum_sampling_task(split_sents_df, args, cl_sample=True):
    task_df_list = []
    task_df_score_list = []
    domain = 'CL'

    group_supoort_df_list = []
    for domain_n in range(args.domain_nums):
        group_supoort_df_tmp = assign_group_by_sentences_sampling(
            signature='support.' + domain,
            args=args,
            info_df=split_sents_df[domain_n].support,
            cl_assign=cl_sample
        )
        group_supoort_df_list.append(group_supoort_df_tmp)

    group_supoort_df = pd.concat(group_supoort_df_list)

    group_query_df_list = []
    for domain_n in range(args.domain_nums):
        group_query_df_tmp = assign_group_by_sentences_sampling(
            signature='query.' + domain,
            args=args,
            info_df=split_sents_df[domain_n].query,
            cl_assign=cl_sample
        )
        group_query_df_list.append(group_query_df_tmp)

    group_query_df = pd.concat(group_query_df_list)
    ...
    for task_id in range(0, min(support_task_num, query_task_num)):
        s_label = 'support.' + domain + '_' + str(task_id)
        q_label = 'query.' + domain + '_' + str(task_id)
        task_df_list.append(group_supoort_df[group_supoort_df.task_group == s_label].append(
            group_query_df[group_query_df.task_group == q_label]).reset_index(drop=True))
    for idx, i in enumerate(task_df_list):
        task_df_score_list.append(i.score.mean())
    task_with_score = list(zip(task_df_list, task_df_score_list))
    # ipdb.set_trace()
...
def split_word_currciulum_task(split_sents_df, support_tokens, query_tokens, general_support_df, general_query_df, cl_sample=True):
    task_df_list = []
    task_df_score_list = []
    task_df_orig_list = []
    domain = 'CL'
    general_support_df_split_list = []
    general_query_df_split_list = []

    for i in range(160):
        tmp_support_df = general_support_df[1000 * i: 1000 * i + 1000]
        tmp_support_df.task_group = ['support.CL_' + str(i)] * 1000
        tmp_query_df = general_query_df[1000 * i: 1000 * i + 1000]
        tmp_query_df.task_group = ['query.CL_' + str(i)] * 1000
        general_support_df_split_list.append(tmp_support_df)
        general_query_df_split_list.append(tmp_query_df)
    ...
    for task_id in range(0, min(support_task_num, query_task_num)):
        s_label = 'support.' + domain + '_' + str(task_id)
        q_label = 'query.' + domain + '_' + str(task_id)
        task_df_orig_list.append(group_supoort_df[group_supoort_df.task_group == s_label].append(group_query_df[group_query_df.task_group == q_label]).reset_index(drop=True))
        task_df_list.append(group_supoort_df[group_supoort_df.task_group == s_label].append(
            group_query_df[group_query_df.task_group == q_label]).append(general_support_df_split_list[task_id]).append(
            general_query_df_split_list[task_id]).reset_index(drop=True))
    for idx,i in enumerate(task_df_orig_list):
        task_df_score_list.append(i.score.mean())
    task_with_score = list(zip(task_df_list, task_df_score_list))
    # ipdb.set_trace()
    if cl_sample:
...
def merge_meta_dev(meta_dev_df_list):
    meta_dev_df_merge = pd.DataFrame({
        'src': [],
        'tgt': [],
        'score': [],
        'task_group': []
    })
    for i in meta_dev_df_list:
        meta_dev_df_merge = meta_dev_df_merge.append(i[0])
    return meta_dev_df_merge
...
def read_tasks_ordered_by_curriculum(domains, source_dictionary, target_dictionary, args,
    ...
    for domain in domains:
        ...
        meta_dev_df_list.append(meta_dev)
    # dev corpus
    # Convert data frames to fairseq tasks
    merge_dev = []
    merge_dev_df = merge_meta_dev(meta_dev_df_list)
    merge_dev.append(merge_dev_df)
    convert_to_fairseq_task_fn = partial(to_fairseq_word_task, args=args, src_dict=source_dictionary, tgt_dict=target_dictionary, sample_df=merge_dev_df)
    meta_dev = list(map(convert_to_fairseq_task_fn, merge_dev, [0]))
    dev_by_domain['CL'] = meta_dev
    print('[{}] Number of dev sentences: {} '.format(domain, len(meta_dev[0].user_data_frame)))

    # ======= Merge meta-train for spliting
    # cl_merge_df = merge_meta_train(meta_train_df_list[1:])
    meta_train_with_score = split_word_curriculum_sampling_task(meta_train_df_list, args, cl_sample=cl_learn)
    meta_train = [i[0] for i in meta_train_with_score]
    meta_train_score = [i[1] for i in meta_train_with_score]
    convert_to_fairseq_task_fn = partial(to_fairseq_train_task, args=args, src_dict=source_dictionary, tgt_dict=target_dictionary)
...
def cli_main(main_fn):
    ...
    options.add_damethod_args(parser)
