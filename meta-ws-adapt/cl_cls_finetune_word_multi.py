...
def to_fairseq_word_task(task, task_score, args, src_dict, tgt_dict, sample_df):
    task_args = deepcopy(args)
    # domain_task = task.task_group.any().split('.')[-1].split('_')[0]
    task_args.train_subset = task_args.train_subset + '.finetune'
    task_args.valid_subset = task_args.valid_subset + '.finetune'
    task_args.test_subset = task_args.test_subset + '.finetune'
...
def load_static_meta_splits(root, domain, meta_dev_subset, meta_test_subset, downstream_train_subset, downstream_valid_subset, downstream_test_subset):
    ...
    meta_test_dir = os.path.join(root, domain, meta_test_subset)
    source_lang = 'en'
    target_lang = 'de'
...
def read_word_tasks_ordered_by_domain(domains, source_dictionary, target_dictionary, args, meta_dev_subset, meta_test_subset, root, downstream_train_subset,downstream_valid_subset, downstream_test_subset, cl_learn):
    tasks_by_domain = OrderedDict()
    dev_by_domain = OrderedDict()
    # rng = np.random.RandomState(seed)
    ft_data_split_list = []
    ...
    for domain in domains:
        ...
        ft_data_split = ft_support_with_buckets.append(meta_test.query)
        ft_data_split = ft_data_split.append(meta_dev.dev)
        ft_data_split_list.append(ft_data_split)
        ft_support_score_list.append(ft_support_with_buckets.score)

    all_df = pd.concat(ft_data_split_list)
    all_score_df = pd.concat(ft_support_score_list)

    # Convert data frames to fairseq tasks
    convert_to_fairseq_task_fn = partial(to_fairseq_word_task, args=args, src_dict=source_dictionary, tgt_dict=target_dictionary, sample_df=all_df)
    # TODO: Pause Here
    ft_task = list(map(convert_to_fairseq_task_fn, [all_df], [all_score_df]))
    tasks_by_domain['finetune'] = ft_task
    # dev_by_domain[domain] = meta_dev
    print('[{}] Number of meta-test tasks: {} '.format(domain, len(ft_task)))
    print('[{}] Number of finetune sentences: {} '.format(domain, len(ft_support_with_buckets)))
    print('[{}] Number of tests sentences: {} '.format(domain, len(meta_test.query)))
...
def prepare_meta_adaptation_tasks(meta_learning_args, downstream_args):
    ...
    tasks_by_domain = read_word_tasks_ordered_by_domain(domains=domains,
...
def main_meta_adaptation(meta_learning_args, downstream_args):
    ...
    # model = tasks_by_domain['finetune'][0].build_model(downstream_args)
